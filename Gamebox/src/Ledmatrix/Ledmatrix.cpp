...
Ledmatrix::Ledmatrix(byte _data, byte _load, byte _clock, byte _num)
{
	data = _data;
	load = _load;
	clock = _clock;
...
void Ledmatrix::setColumn(byte col, byte value)
{
	int n = col / 8;
	int c = col % 8;
	digitalWrite(load, LOW);
...
void Ledmatrix::setColumnAll(byte col, byte value)
{
	digitalWrite(load, LOW);
	for (int i=0; i<num; i++)
	{
...
void Ledmatrix::setDot(byte col, byte row, byte value)
{
...
void Ledmatrix::writeSprite(int x, int y, const byte* sprite)
{
	int w = sprite[0];
	int h = sprite[1];
...
void Ledmatrix::shiftLeft(bool rotate, bool fill_zero)
{
	byte old = buffer[0];
	int i;
	for (i=0; i<80; i++)
...
void Ledmatrix::shiftRight(bool rotate, bool fill_zero)
{
	int last = num*8-1;
...
void Ledmatrix::shiftUp(bool rotate)
{
...
void Ledmatrix::shiftDown(bool rotate)
{
	for (int i=0; i<num*8; i++)
	{
		bool b = buffer[i] & 128;
